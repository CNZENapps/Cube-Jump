...
let obstacleSpeed = 5; // السرعة الابتدائية للعائق (بالثواني)
...
// مراقبة "الغلطة" (عودة الخلفية إلى نقطة الصفر)
let checkGlitchInterval = setInterval(() => {
    let gameContainer = document.getElementById('gameContainer');
    let bgPosition = window.getComputedStyle(gameContainer).backgroundPositionX;

    if (bgPosition === "0px") { // اكتشاف "الغلطة"
...
function increaseObstacleSpeed() {
    ...
    obstacleSpeed -= 0.5; // تقليل مدة الحركة بمقدار 0.5 ثانية
    if (obstacleSpeed < 1) obstacleSpeed = 1; // الحد الأدنى للسرعة
    obstacle.style.animationDuration = obstacleSpeed + "s"; // تحديث سرعة العائق
}
...
setInterval(() => {
    ...
    if (
        ...
        clearInterval(checkGlitchInterval); // إيقاف مراقبة "الغلطة"
